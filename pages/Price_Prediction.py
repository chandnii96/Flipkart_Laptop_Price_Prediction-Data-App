...
IMAGE_PATH =os.path.join(dir_of_interest, "images", "laptop1.jpg")
...
brand = st.selectbox(':red[Select Laptop Brand]',(df['Company'].unique()))
    

operating_system=st.selectbox(':red[Select Operating System]',(df['Operating System'].unique()))


processor=st.selectbox(':red[Select Processor Type]',(df['Processor Type'].unique()))
    

ram_type=st.selectbox(':red[Select RAM Type]',(df['RAM Type'].unique()))
    


ram_size=st.selectbox( ':red[Select RAM Size]',(df['RAM size'].unique()))
    

disc_type=st.selectbox(':red[Select DISC Type]',(df['Disc Type'].unique()))
    


disc_size=st.selectbox(':red[Select DISC Size]',(df['Disc Size'].unique()))
